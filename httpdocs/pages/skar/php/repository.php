...
{	
	...
	function findUser( $dn, $pass )
	{
		...
		$sqlResult = mysql_query( $query, $this->connection )
			or die ( 'Repository->findUser() - error: ' . mysql_error() );
	...
	{
		return $this->_fetchResources( "SELECT * FROM `resource` WHERE `id` in ( SELECT `source` FROM `link` WHERE `target` = $id )", $this->connection );
	}
	...
	function listResources( $parent, $types, $orderByClause )
	{
		...
		$resources = $this->_fetchResources( $sqlQuery, $this->connection );		
	...
	function _fetchResource( $query )
	{
		...
		$sqlResult = mysql_query( $query, $this->connection )
			or die ( 'Repository->fetchResource() - error: ' . mysql_error() );
	...
	function _fetchResources( $query )
	{
		...
		$sqlResult = mysql_query( $query, $this->connection )
			or die ( 'Repository->_fetchResources() - error: ' . mysql_error() );
