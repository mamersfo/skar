...
function home()
{
	...
	if ( $( 'content-intro' ).style.visibility == 'hidden' )
	{
		$( 'content-intro' ).style.visibility = 'visible';
	}
	...
	if ( result != null )
	{
		...
		else
		{
			...
			if ( currentMenu.folders.length > 0 )
			{
				...
				$( 'content-image' ).style.visibility = 'hidden';
...
{
	$( 'content-intro' ).style.visibility = 'hidden';
...
function showProperty( property )
{
	...
	{
		image.src = property.thumbnails[0].uri;
		
		$( 'content-image' ).innerHTML = '';
		...
		$( 'content-image' ).style.backgroundImage = 'url("' + image.src + '")';
	
		$( 'content-image' ).style.backgroundRepeat = 'no-repeat';	

		setOpacity( $('content-image'), 90 );
	}
	else
	{
	...
	if ( property.backgrounds != null )
	{
		...
		for ( var i=0; i < property.backgrounds.length; i++ )
		{
			backgrounds[i] = property.backgrounds[i];

			( new Image() ).src = backgrounds[i].uri;
		}
	}
}
